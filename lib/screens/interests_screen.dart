...
class _InterestScreenState extends State<InterestScreen> with SingleTickerProviderStateMixin {
  ...
  List<UserModel> _tappedMeUsers = []; // For 'Taps' tab (formerly 'Interested In Me')
  ...
  Future<void> _fetchUsersWhoTappedMe({bool isLoadMore = false}) async {
    if (!isLoadMore) {
      ...
      if (!_hasMoreTapped || _isLoadingTapped) return; // Prevent multiple simultaneous loads
      setState(() {
        _isLoadingTapped = true;
      });
    }
    ...
    try {
      ...
      final List<UserModel> fetchedUsers = await ApiService.getPeople(
        ...
        acceptsNsfwPics: false, // Example filter, adjust as needed
      );
  ...
  void _onTappedScroll() {
    if (_tappedScrollController.position.pixels == _tappedScrollController.position.maxScrollExtent && _hasMoreTapped && !_isLoadingTapped) {
  ...
  Future<void> _fetchUsersWhoViewedMe({bool isLoadMore = false}) async {
    if (!isLoadMore) {
      ...
      if (!_hasMoreViews || _isLoadingViewedMe) return; // Prevent multiple simultaneous loads
      setState(() {
        _isLoadingViewedMe = true;
      });
    }
    ...
    try {
      ...
      final List<UserModel> fetchedUsers = await ApiService.getPeople(
        pageNumber: _currentPageViews,
        pageSize: _pageSizeViews,
  ...
  void _onViewsScroll() {
    if (_viewsScrollController.position.pixels == _viewsScrollController.position.maxScrollExtent && _hasMoreViews && !_isLoadingViewedMe) {
      ...
      body: TabBarView(
        ...
        children: [
          ...
          _isLoadingViewedMe && _viewedMeUsers.isEmpty
              ...
              : _errorMessageViewedMe != null
              ? Center(child: Text(_errorMessageViewedMe!, style: const TextStyle(color: Colors.red)))
            ...
            child: GridView.builder(
              ...
              itemCount: _viewedMeUsers.length + (_hasMoreViews ? 1 : 0),
          ...
          _isLoadingTapped && _tappedMeUsers.isEmpty
              ? const Center(child: CircularProgressIndicator(color: Colors.white))
              : _errorMessageTapped != null
              ? Center(child: Text(_errorMessageTapped!, style: const TextStyle(color: Colors.red)))
            ...
            child: ListView.builder( // Changed to ListView.builder
              ...
              padding: const EdgeInsets.all(8.0),
              itemCount: _tappedMeUsers.length + (_hasMoreTapped ? 1 : 0),
              itemBuilder: (context, index) {
                ...
                return _buildUserListItem(context, user); // Using new list item builder
              },
            ),
          ),
        ],
  ...
  Widget _buildUserGridItem(BuildContext context, UserModel user) {
    final imageUrl = user.imageUrls.isNotEmpty
        ? user.imageUrls[0]
        : 'https://placehold.co/200x300/000000/FFFFFF?text=No+Image'; // Placeholder if no image
  ...
  Widget _buildUserListItem(BuildContext context, UserModel user) {
    final imageUrl = user.imageUrls.isNotEmpty
        ? user.imageUrls[0]
        : 'https://placehold.co/100x100/000000/FFFFFF?text=No+Image'; // Placeholder if no image
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text((user.distance.toString()), // Use formatted distance
